...
function functionbtnHistory() {
    var noOfDays = 15;             //change number of days here upto which history needs tobe shown
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (i = 0; i < data.length; i++) {
                ...
                $('#statusTableForRecent tr:last').after('<tr class="rowForComponents"><td>' + data[i].DBComponentName + '</td><td><img src="" id="componentStatusIconForComponent' + componentId+'" style="height:15px; width:15px;"></img></td><td colspan="4"><div id="divForComponentOnRecent' + componentId + '" style="width:100%; height:10px"></div></td></tr>');
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        for (j = 0; j < data.length; j++) {
                            ...
                            if (j == data.length - 1)
                            {
                                 var logId = data[j].mLogId;
                                if (data[j].mStatus == "Operational") {
                                    $("#componentStatusIconForComponent" + componentId).attr('src', '../../Content/Images/operational.png');
                                }
                                else if (data[j].mStatus == "Performance Degradation") {
                                    $("#componentStatusIconForComponent" + componentId).attr('src', '../../Content/Images/performance_degradation.png');
                                    $("#componentStatusIconForComponent" + componentId).removeClass('clickable');
                                    $("#componentStatusIconForComponent" + componentId).addClass('clickable');
                                    $("#componentStatusIconForComponent" + componentId).click(function () {
                                        getDetailsOfIncident(logId);

                                    });
                                }
                                else {
                                    $("#componentStatusIconForComponent" + componentId).attr('src', '../../Content/Images/service_disruption.png');
                                    $("#componentStatusIconForComponent" + componentId).removeClass('clickable');
                                    $("#componentStatusIconForComponent" + componentId).addClass('clickable');
                                    $("#componentStatusIconForComponent" + componentId).click(function () {
                                        getDetailsOfIncident(logId);
                            ...
                            if (data[j].mLogId != -1) {
                                $("#divForComponentOnRecent" + componentId).append('<div class="clickable" id="' + data[j].mLogId + '" style="background-color:' + color + '; width:' + eachUnit * parseInt(data[j].mDiff) + '%; height:10px;float:left;" onclick ="getDetailsOfIncident(' + data[j].mLogId + ')" ></div>');
                            }
                            else
...
function getDetailsOfIncident(componentId) {
    $("#divForDetails").val("");
    $("#divForDetails").hide();
    if (componentId != -1) {
        $.ajax({
            url: '/User/getDetailsOfIncident',
            data: { pstrLogId: componentId },
            cache: false,
            async: false,
            datatype: 'Json',
            success: function (data) {
                ...
                if (data.DBDateTimeEnd != null)
                    ...
                    $.ajax({
                        ...
                        type: 'post',
                        datatype: 'Json',
                        data: { pstrComponentId: data.DBCSId },
                        async: false,
                        cache: false,
                        success: function (data) {
                            ...
                            //alert("got expected end date successfully");
                        },
                        error: function () {
                    ...
                    document.getElementById("spanForEndTime").innerHTML = expectedEndTime;
                }
        ...
        $("#divForDetails").show('slide', { direction: 'right' }, 500);
    }
}
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        for (j = 0; j < data.length; j++) {
                            ...
                            if (data[j].mLogId != -1) {
                                $("#divForComponentOnHistory" + componentId).append('<div class="clickable" id="' + data[j].mLogId + '" style="background-color:' + color + '; width:' + eachUnit * parseInt(data[j].mDiff) + '%; height:10px;float:left;" onclick ="getDetailsOfIncident(' + data[j].mLogId + ')" ></div>');
                            }
                            else
